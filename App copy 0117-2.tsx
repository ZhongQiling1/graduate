...
function App() {
  ...
  const generateEnhancedDataDrivenPrompt = async (
    ...
    const getComparisonObjects = () => {
      if (isDualMode) {
        const fileA = analysis.fileA_analysis?.industry_domain || '文件A'
        const fileB = analysis.fileB_analysis?.industry_domain || '文件B'
        return [fileA, fileB]
      } else {
        return (
          analysis.comparisonItems?.[0]?.split(' vs ') || ['实体A', '实体B']
        )
      }
    }
...
左侧：暖色调的${
          sizeRatio > 1 ? '高大' : '适中'
        }物体堆代表${objectA}（${valueA}${unit}）
右侧：冷色调的${
          sizeRatio < 1 ? '高大' : '适中'
        }物体堆代表${objectB}（${valueB}${unit}）
专业摄影风格，4K画质。`
        break
      case 2:
        prompt = `建筑建设对比图像，展示${objectA}与${objectB}的对比。
左侧：暖色调建筑代表${objectA}（${valueA}${unit}）
右侧：冷色调建筑代表${objectB}（${valueB}${unit}）
建筑摄影风格。`
        break
      case 3:
        prompt = `密度分布对比图，展示${objectA}与${objectB}的差异。
左侧：暖色调密集区域代表${objectA}
右侧：冷色调稀疏区域代表${objectB}
专业数据可视化风格。`
        break
    }
  ...
  const getEnhancedDataDrivenFallbackPrompt = (
    ...
    const promptTemplates = {
      1: `实体隐喻对比图像：左侧暖色调物体堆代表${objectA}，右侧冷色调物体堆代表${objectB}。写实风格。`,
      2: `结构隐喻对比图像：左侧暖色调建筑代表${objectA}，右侧冷色调建筑代表${objectB}。建筑摄影。`,
      3: `密度分布对比图：左侧暖色调密集区域代表${objectA}，右侧冷色调稀疏区域代表${objectB}。可视化风格。`
    }
  ...
  const getFallbackContrastPrompt = (
    ...
    const getComparisonObjects = () => {
      if (isDualMode) {
        const fileA = analysis.fileA_analysis?.industry_domain || '特征A'
        const fileB = analysis.fileB_analysis?.industry_domain || '特征B'
        return [fileA, fileB]
      } else {
        return (
          analysis.comparisonItems?.[0]?.split(' vs ') || ['特征A', '特征B']
        )
      }
    }
    ...
    const promptTemplates = {
      1: `左侧暖色调物体堆代表${objectA}，右侧冷色调物体堆代表${objectB}。写实风格。`,
      2: `左侧暖色调建筑代表${objectA}，右侧冷色调建筑代表${objectB}。建筑摄影。`,
      3: `密度对比图像：左侧暖色调区域密集，右侧冷色调区域稀疏。可视化风格。`
    }
...
<div class="space-y-6 text-gray-700">
  ...
  <div class="text-2xl font-bold text-gray-900 flex items-center">
    <span class="mr-2">📊</span> 数据对比分析报告（${metaphorTypeName}）${
      analysis.industry_analysis?.detected_industry
        ? `<span class="ml-4 text-sm font-normal text-purple-600">行业：${analysis.industry_analysis.detected_industry}</span>`
        : ''
    }
  </div>`
    // AI生成的内容区域
