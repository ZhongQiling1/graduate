...
import {
  ...
  selectMetaphorObjects
} from './industryMetaphorUtils'
...
function App() {
  ...
  const generateEnhancedDataDrivenPrompt = async (
    ...
    // 1. 智能识别行业
    const industryDetection = detectIndustryFromData(analysis)

    // 2. 根据行业选择喻体
    const metaphorObjects = selectMetaphorObjects(
      industryDetection.industry,
      metaphorType.visualType,
    ...
    // 8. 直接使用comparisonItems获取对比对象名称
    const objectA = analysis.comparisonItems[0]?.split(' vs ')[0] || labelA
    const objectB = analysis.comparisonItems[0]?.split(' vs ')[1] || labelB

    // 9. 根据隐喻类型构建专业提示词 - 使用智能选择的物体
    let prompt = ''

    switch (metaphorType.id) {
      case 1: {
        // 实体隐喻 - 使用行业特定的物体堆叠
        prompt = `一张专业的数据对比图像，使用${industryDetection.industry.replace(
          '_',
          '/'
        )}行业喻体${metaphorObjects.primaryObject}。左侧：${
          metaphorObjects.colorScheme.warm[0]
        }调的${sizeRatio > 1 ? '高大' : '适中'}${
          metaphorObjects.primaryObject
        }代表${objectA}（数值${valueA}${unit}），物体上带有${objectA}的特征标识；右侧：${
          metaphorObjects.colorScheme.cool[0]
        }调的${sizeRatio < 1 ? '高大' : '适中'}${
          metaphorObjects.primaryObject
        }代表${objectB}（数值${valueB}${unitB}），物体上带有${objectB}的特征标识。背景简洁（浅灰色渐变），光线自然，通过视觉元素和特征标识清晰传达对比关系。专业摄影风格，4K画质。`
        break
      }

      case 2: {
        // 结构隐喻 - 建筑建设过程，用吊车数量体现增长快慢
        const buildingType = metaphorObjects.primaryObject

        // 获取趋势斜率（判断增长快慢）
        const slopeA = numericalFeatures.trend_slopes?.entityA || 0.15
        const slopeB = numericalFeatures.trend_slopes?.entityB || 0.08
        ...
        // 根据斜率判断吊车数量（增长快慢）
        const getCraneCount = (slope: number): number => {
          if (slope > 0.2) return 4 // 非常快速增长
          if (slope > 0.15) return 3 // 快速增长
          if (slope > 0.1) return 2 // 稳步增长
          if (slope > 0.05) return 1 // 缓慢增长
          return 0 // 基本不增长
        }

        const craneCountA = getCraneCount(slopeA)
        const craneCountB = getCraneCount(slopeB)

        // 吊车分布描述
        const getCraneDescription = (count: number, slope: number): string => {
          if (count === 0) return '无吊车，仅基础施工'
          if (count === 1)
            return `1台吊车在工作，${slope > 0.08 ? '匀速施工' : '间歇施工'}`
          if (count === 2)
            return `2台吊车同时作业，${slope > 0.12 ? '高效协同' : '有序配合'}`
          if (count === 3) return `3台吊车密集作业，施工速度很快`
          return `4台或更多吊车快速运转，施工进度飞速推进`
        }

        // 建筑完成度（基于当前值占最大值的比例）
        const completionA = Math.min(
          100,
          (valueA /
            ((numericalFeatures.values?.entityA as { max?: number })?.max ??
              valueA * 1.5)) *
            100
        )

        const completionB = Math.min(
          100,
          (valueB /
            ((numericalFeatures.values?.entityB as { max?: number })?.max ??
              valueB * 1.5)) *
            100
        )

        // 施工阶段描述
        const getConstructionStage = (completion: number): string => {
          if (completion < 20) return '地基施工阶段'
          if (completion < 40) return '基础框架阶段'
          if (completion < 60) return '主体结构阶段'
          if (completion < 80) return '封顶装修阶段'
          return '接近完工阶段'
        }

        const stageA = getConstructionStage(completionA)
        const stageB = getConstructionStage(completionB)
        ...
        const getConstructionDetails = (
          ...
          if (count === 0) {
            return completion < 30 ? '少量工人进行基础施工' : '工人进行内部装修'
          }
          if (count === 1) {
            return completion < 50 ? '吊车吊装建筑材料' : '吊车协助内部设备安装'
          }
          if (count === 2) {
            return '两台吊车分别负责不同区域的建材运输'
          }
          if (count === 3) {
            return '多台吊车协同作业，建筑材料快速流转'
          }
          return '密集的吊车群高效运转，施工场地繁忙有序'
        }

        // 吊车位置分布（增加真实感）
        const getCranePositions = (count: number): string => {
          if (count === 1) return '位于建筑一侧'
          if (count === 2) return '分别位于建筑两侧'
          if (count === 3) return '围绕建筑三角分布'
          return '围绕建筑四周密集分布'
        }

        // 构建详细的提示词
        prompt = `一张建筑建设对比图像，清晰展示${objectA}与${objectB}的数据规模与增长趋势对比。

左侧（${objectA}）：
- 建筑：${heightA}的${buildingType}，${stageA}，完成度约${completionA.toFixed(
          0
        )}%
- 吊车：${getCraneDescription(craneCountA, slopeA)}，${getCranePositions(
          craneCountA
        )}
- 施工细节：${getConstructionDetails(craneCountA, completionA)}
- 增长趋势：季度增长${(slopeA * 100).toFixed(
          1
        )}%（通过${craneCountA}台吊车体现）
- 数据规模：${valueA}${unit}（通过建筑高度体现）
- 视觉特征：暖色调建筑， 黄色吊车，带有${objectA}的标识

右侧（${objectB}）：
- 建筑：${heightB}的${buildingType}，${stageB}，完成度约${completionB.toFixed(
          0
        )}%
- 吊车：${getCraneDescription(craneCountB, slopeB)}，${getCranePositions(
          craneCountB
        )}
- 施工细节：${getConstructionDetails(craneCountB, completionB)}
- 增长趋势：季度增长${(slopeB * 100).toFixed(
          1
        )}%（通过${craneCountB}台吊车体现）
- 数据规模：${valueB}${unitB}（通过建筑高度体现）
- 视觉特征：冷色调建筑，蓝色吊车，带有${objectB}的标识
...
3. 左侧暖色调，右侧冷色调
4. 真实建筑摄影风格，施工现场细节丰富
5. 吊车、脚手架、建筑材料等施工元素清晰可见
6. 蓝天背景，自然光影，4K画质
7. 建筑上带有各自的特征标识，但不过分突兀`
        break
      }
      ...
      case 3: {
        // 方位隐喻 - 密度分布对比
        // 计算密度对比描述
        const densityRatio = sizeRatio
        const isAMuchLarger = densityRatio > 2 // A远大于B
        const isALarger = densityRatio > 1 // A大于B
        const isSimilar = densityRatio >= 0.9 && densityRatio <= 1.1 // 相近

        // 左侧描述（A）
        let leftDensityDesc = '均匀分布'
        let leftArrangementDesc = '规则排列'
        if (isAMuchLarger) {
          leftDensityDesc = '极其密集'
          leftArrangementDesc = '紧密堆积'
        } else if (isALarger) {
          leftDensityDesc = '密集'
          leftArrangementDesc = '紧凑排列'
        }

        // 右侧描述（B）
        let rightDensityDesc = '均匀分布'
        let rightArrangementDesc = '规则排列'
        if (isSimilar) {
          rightDensityDesc = '同样密集'
          rightArrangementDesc = '相似排列'
        } else if (densityRatio < 0.5) {
          rightDensityDesc = '非常稀疏'
          rightArrangementDesc = '松散分布'
        } else if (densityRatio < 1) {
          rightDensityDesc = '稀疏'
          rightArrangementDesc = '分散排列'
        }

        // 构建具体、量化的提示词
        prompt = `一张清晰的密度分布对比图，使用${industryDetection.industry.replace(
          '_',
          '/'
        )}行业元素${
          metaphorObjects.primaryObject
        }展示${objectA}与${objectB}的数据差异。

左侧${metaphorObjects.colorScheme.warm[0]}区域：${leftDensityDesc}的${
          metaphorObjects.primaryObject
        }，每个${metaphorObjects.primaryObject}带有${objectA}的特征标识。${
          metaphorObjects.primaryObject
        }数量约为${
          Math.round(valueA / 100) * 100
        }个，${leftArrangementDesc}，间距${
          isAMuchLarger ? '极小' : '较小'
        }，几乎看不到空隙。

右侧${metaphorObjects.colorScheme.cool[0]}区域：${rightDensityDesc}的${
          metaphorObjects.primaryObject
        }，每个${metaphorObjects.primaryObject}带有${objectB}的特征标识。${
          metaphorObjects.primaryObject
        }数量约为${
          Math.round(valueB / 100) * 100
        }个，${rightArrangementDesc}，间距${
          densityRatio < 0.5 ? '很大' : '较大'
        }，有明显的空白区域。

背景：简洁的深灰色网格，帮助观察分布密度
光线：均匀平面光，无阴影干扰
视觉要求：密度对比必须一目了然，物体清晰可数，特征标识可识别但不喧宾夺主。`
        break
      }
    }

    // 10. 添加技术要求
    prompt += ` 技术要求：高清分辨率，真实光影，焦点清晰，色彩准确，构图平衡，确保两个对象的特征标识清晰可识别但不过分突兀，对比关系一目了然。`
  ...
  const getEnhancedDataDrivenFallbackPrompt = (
    ...
    const isGrowth = dataType.includes('增长') || dataType.includes('趋势')
    ...
    const promptTemplates = {
      1: `实体隐喻对比图像：左侧暖色调${
        ...
        isSales ? '金币堆' : '具体物品堆'
      }代表${objectA}（${valueA}${unit}）；右侧冷色调${
        sizeRatio < 1 ? '高大' : '适中'
      }的带有${objectB}特征标识的${
        isSales ? '金币堆' : '具体物品堆'
      }代表${objectB}（${valueB}${unit}）。特征标识清晰可辨，堆叠整齐，背景简洁，光线自然。写实风格，4K画质。`,

      2: `结构隐喻对比图像：左侧暖色调${
        sizeRatio > 1 ? '快速建造' : '稳步建造'
      }的融入${objectA}特征标识的${
        sizeRatio > 1.5 ? '摩天大楼' : '建筑框架'
      }代表${objectA}的${
        isGrowth ? '增长趋势' : '数据对比'
      }（${valueA}${unit}）；右侧冷色调${
        sizeRatio < 1 ? '稳步建造' : '基础建设'
      }的融入${objectB}特征标识的${
        sizeRatio < 0.8 ? '古典建筑' : '现代结构'
      }代表${objectB}的${
        isGrowth ? '增长趋势' : '数据对比'
      }（${valueB}${unit}）。特征标识自然融入建筑，建筑摄影风格。`,

      3: `密度分布对比图：左侧暖色调区域${
        sizeRatio > 1 ? '极其密集' : '密集'
      }排列的${
        isRelation ? '小连接点' : '小球'
      }（每个带有${objectA}特征标识），数量约${
        Math.round(valueA / 100) * 100
      }个，排列紧凑，几乎没有空隙；右侧冷色调区域${
        sizeRatio < 0.5 ? '非常稀疏' : '稀疏'
      }分布的${
        isRelation ? '小节点' : '小球'
      }（每个带有${objectB}特征标识），数量约${
        Math.round(valueB / 100) * 100
      }个，分布松散，有明显的空白区域。背景为浅灰色网格，密度差异一目了然。专业数据可视化风格，4K分辨率。`
    }
  ...
  const getFallbackContrastPrompt = (
    ...
    const promptTemplates = {
      ...
      }的带有${objectA}特征标识的物体堆代表${objectA}，体现${
        volatilityFeature.includes('稳定') ? '稳定积累' : '波动变化'
      }；右侧冷色调${
        sizeFeature.includes('B') ? '高大' : '适中'
      }的带有${objectB}特征标识的物体堆代表${objectB}，体现${
        volatilityFeature.includes('波动') ? '动态变化' : '稳定存储'
      }；特征标识清晰可识别，整体写实风格，高分辨率，真实质感。`,

      2: `左侧暖色调${
        sizeFeature.includes('A') ? '快速搭建' : '稳步建造'
      }的融入${objectA}特征标识的建筑结构代表${objectA}，体现${
        volatilityFeature.includes('稳定') ? '坚固有序' : '动态发展'
      }；右侧冷色调${
        sizeFeature.includes('B') ? '高大框架' : '基础结构'
      }的融入${objectB}特征标识的建筑结构代表${objectB}，体现${
        volatilityFeature.includes('波动') ? '变化过程' : '稳定成长'
      }；特征标识自然融入建筑，专业建筑摄影。`,

      3: `密度对比图像：左侧暖色调区域${
        sizeFeature.includes('A') ? '紧密堆积' : '规则排列'
      }的带有${objectA}特征标识的小方块，${
        sizeFeature.includes('A') ? '几乎看不到空隙' : '间距均匀'
      }；右侧冷色调区域${
        sizeFeature.includes('B') ? '松散分布' : '均匀散布'
      }的带有${objectB}特征标识的小方块，${
        sizeFeature.includes('B') ? '有明显空白区域' : '间距规则'
      }。通过排列密度直观展示数据差异。`
    }
...
现在请开始你的分析：`
      ...
      setNarrativeResult(formattedResponse) // 立即设置文本结果
                ...
                <div className="mt-6 space-y-6">
                  ...
                  {!narrativeResult && (
                    <div className="rounded-lg border border-blue-200 bg-gradient-to-br from-blue-50 to-cyan-50 p-4">
                      ...
                      {/* 对比项目 */}
                      {dataAnalysis.comparisonItems.length > 0 && (
                        <div className="mb-3">
                          <div className="mb-1 text-sm font-medium text-gray-700">
                            对比项目：
                          </div>
                          <div className="flex flex-wrap gap-1">
                            {dataAnalysis.comparisonItems.map((item, index) => (
                              <span
                                key={index}
                      ...
                      <p className="mt-2 text-sm text-gray-600">
                        已识别 {dataAnalysis.comparisonItems.length}{' '}
                        个对比项，请点击上方按钮生成对比图像
                      </p>
                    </div>
                  )}
