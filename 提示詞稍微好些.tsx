...
function App() {
  ...
  // 生成对比提示词 - 修复metaphorTypeName未定义问题
  const generateContrastPromptByAI = async (
    analysis: DataAnalysisResult,
    metaphorType: (typeof metaphorTypes)[0] // 新增：接收完整的隐喻类型对象
  ): Promise<string> => {
    // 基于Marks+Channels逻辑构建结构化提示词
    const prompt = `你是专业视觉隐喻设计师，基于概念隐喻理论和Marks+Channels可视化逻辑生成数据对比图像提示词。
...
【隐喻规则】
隐喻类型：${metaphorType.name} // 修复：用metaphorType.name代替metaphorTypeName
...
【严格生成要求】
1. 必须生成单一张图像，体现数据对比关系，严格遵循${
      metaphorType.name
    }原理 // 修复：替换为metaphorType.name
    ...
    try {
      ...
      for (const word of sensitiveWords) {
        if (lowerContent.includes(word.toLowerCase())) {
          ...
          return getFallbackContrastPrompt(analysis, metaphorType) // 同步传metaphorType
        }
      }
      ...
      if (content.length < 80) {
        // 长度不足时补充基础要求
        content += ` 整体极简商务风，高分辨率，真实物品质感，左侧暖色调代表${
          analysis.comparisonItems[0]?.split(' vs ')[0] || '特征A'
        }，右侧冷色调代表${
          analysis.comparisonItems[0]?.split(' vs ')[1] || '特征B'
        }，左右对比清晰，留白充足。`
...
"左侧暖色调大尺寸规整形态的[物品A]代表[特征A]，体现[数据特征]；右侧冷色调小尺寸动态形态的[物品B]代表[特征B]，体现[数据特征]；整体极简商务风，高分辨率，真实质感，左右对比清晰，留白充足。"`
      ...
      // 6. 生成对比提示词（全链路存储）
      rawContrastPrompt = await generateContrastPromptByAI(
        analysis,
        metaphorType
      )
