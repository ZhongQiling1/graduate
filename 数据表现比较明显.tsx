...
function App() {
  ...
  // 数据驱动的图像提示词生成函数
  const generateDataDrivenPrompt = async (
    ...
    const numericalFeatures = analysis.numerical_features || {
      ...
      volatility_ratio: 2.3,
      values: {
        entityA: { mean: 1500, current: 1800 },
        entityB: { mean: 850, current: 1000 }
      }
    }
    ...
    // 计算视觉元素尺寸（基于像素或比例）
    const calculateVisualSize = (value: number): string => {
      const percentage = ((value - minValue) / (maxValue - minValue)) * 100
      if (percentage > 75) return '非常大'
      if (percentage > 50) return '大'
      if (percentage > 25) return '中等'
      return '小'
    }

    const sizeA = calculateVisualSize(valueA)
    const sizeB = calculateVisualSize(valueB)

    // 波动性特征
    const volatility = analysis.characteristics?.volatility || ''
    const volatilityA =
      volatility.includes('A') && volatility.includes('稳定') ? '稳定' : '波动'
    const volatilityB =
      volatility.includes('B') && volatility.includes('稳定') ? '稳定' : '波动'

    // 根据隐喻类型生成具体的可视化提示
    const getVisualizationByMetaphor = () => {
      const basePrompt = {
        1: `两个并列的人物形象，左侧人物${sizeA}且${
          volatilityA === '稳定' ? '站立稳定' : '动作活跃'
        }，代表${entityA}（数值：${valueA}）；右侧人物${sizeB}且${
          volatilityB === '稳定' ? '站立稳定' : '动作活跃'
        }，代表${entityB}（数值：${valueB}）。`,
        2: `两座并列的建筑物，左侧建筑${sizeA}且${
          volatilityA === '稳定' ? '结构规整' : '结构复杂'
        }，高度与${valueA}成正比；右侧建筑${sizeB}且${
          volatilityB === '稳定' ? '结构规整' : '结构复杂'
        }，高度与${valueB}成正比。`,
        3: `两个相邻的空间区域，左侧区域${sizeA}且${
          volatilityA === '稳定' ? '布局整齐' : '布局有机'
        }，元素密度反映${valueA}；右侧区域${sizeB}且${
          volatilityB === '稳定' ? '布局整齐' : '布局有机'
        }，元素密度反映${valueB}。`,
        4: `两种并列的纹理图案，左侧纹理${sizeA}且${
          volatilityA === '稳定' ? '规律平缓' : '不规则起伏'
        }，纹理密度对应${valueA}；右侧纹理${sizeB}且${
          volatilityB === '稳定' ? '规律平缓' : '不规则起伏'
        }，纹理密度对应${valueB}。`
      }

      return basePrompt[metaphorType.id] || basePrompt[1]
    }

    // 构建数据驱动的提示词
    const dataDrivenPrompt = `
# 数据可视化图像生成指令

## 📊 数据信息
- **${entityA}**: ${valueA} 单位（${sizeA}）
- **${entityB}**: ${valueB} 单位（${sizeB}）
- **大小比例**: ${sizeRatio.toFixed(1)}:1

## 🎨 视觉映射规则
1. **尺寸映射**: 数值越大，视觉元素尺寸越大（高度/宽度/数量）
   - ${valueA} → ${sizeA}尺寸
   - ${valueB} → ${sizeB}尺寸

2. **稳定性映射**:
   - ${volatilityA === '稳定' ? '稳定趋势' : '波动趋势'} → ${
     volatilityA === '稳定' ? '规整/简洁形态' : '动态/复杂形态'
   }
...
## 📐 具体要求
${getVisualizationByMetaphor()}

## 🎯 图像要求
- 左侧暖色区域代表${entityA}，右侧冷色区域代表${entityB}
- 尺寸差异必须清晰可见（${sizeRatio.toFixed(1)}倍关系）
- 添加极简的数据标签：左侧"${entityA}: ${valueA}"，右侧"${entityB}: ${valueB}"
- 商务数据可视化风格，简洁专业
- 高分辨率，清晰对比
- 无多余装饰，焦点在数据对比
`
    ...
    try {
      ...
      const response = await fetch('http://localhost:3001/api/generate-text', {
        ...
        body: JSON.stringify({
          prompt: `将以下数据可视化指令优化为80-120字的文生图提示词：\n\n${dataDrivenPrompt}`
        })
      })
    ...
    // 本地生成的精确提示词
    return `一张数据对比可视化图像：左侧暖色调${sizeA}${
      metaphorType.name === '实体隐喻'
        ? '人物'
        : metaphorType.name === '结构隐喻'
          ? '建筑'
          : '区域'
    }代表${entityA}（数值${valueA}），${
      volatilityA === '稳定' ? '形态规整稳定' : '形态动态变化'
    }; 右侧冷色调${sizeB}${
      metaphorType.name === '实体隐喻'
        ? '人物'
        : metaphorType.name === '结构隐喻'
      ...
      volatilityB === '稳定' ? '形态规整稳定' : '形态动态变化'
    }. 尺寸比例为${sizeRatio.toFixed(
      1
    )}:1，图像下方标注具体数值，简洁商务风格，高分辨率，数据可视化清晰.`
  }

  // 数据驱动的备用提示词
  const getDataDrivenFallbackPrompt = (
    analysis: DataAnalysisResult,
    metaphorType: (typeof metaphorTypes)[0]
  ): string => {
    const numericalFeatures = analysis.numerical_features || {
      size_ratio: 1.8,
      values: {
        entityA: { current: 1800 },
        entityB: { current: 1000 }
      }
    }
    ...
    const sizeA = valueA > valueB ? '大尺寸' : '小尺寸'
    const sizeB = valueB > valueA ? '大尺寸' : '小尺寸'

    const promptTemplates = {
      1: `左侧暖色调${sizeA}人物代表${entityA}（${valueA}单位），${
        analysis.characteristics?.volatility?.includes('稳定')
          ? '站立稳定'
          : '动作活跃'
      }; 右侧冷色调${sizeB}人物代表${entityB}（${valueB}单位），${
        analysis.characteristics?.volatility?.includes('波动')
          ? '动作活跃'
          : '站立稳定'
      }. 人物大小比例${sizeRatio.toFixed(1)}:1，标注具体数值.`,
      2: `左侧暖色调${sizeA}建筑代表${entityA}（${valueA}单位），${
        analysis.characteristics?.volatility?.includes('稳定')
          ? '结构规整'
          : '结构复杂'
      }; 右侧冷色调${sizeB}建筑代表${entityB}（${valueB}单位），${
        analysis.characteristics?.volatility?.includes('波动')
          ? '结构复杂'
          : '结构规整'
      }. 建筑高度与数值成正比，标注具体数值.`,
      3: `左侧暖色调${sizeA}区域代表${entityA}（${valueA}单位），${
        analysis.characteristics?.volatility?.includes('稳定')
          ? '布局整齐'
          : '布局有机'
      }; 右侧冷色调${sizeB}区域代表${entityB}（${valueB}单位），${
        analysis.characteristics?.volatility?.includes('波动')
          ? '布局有机'
          : '布局整齐'
      }. 元素密度反映数值大小，标注具体数值.`,
      4: `左侧暖色调${sizeA}纹理代表${entityA}（${valueA}单位），${
        analysis.characteristics?.volatility?.includes('稳定')
          ? '规律平缓'
          : '不规则起伏'
      }; 右侧冷色调${sizeB}纹理代表${entityB}（${valueB}单位），${
        analysis.characteristics?.volatility?.includes('波动')
          ? '不规则起伏'
          : '规律平缓'
      }. 纹理密度对应数值大小，标注具体数值.`
    ...
    return promptTemplates[metaphorType.id] || promptTemplates[1]
  }

  // 在渲染结果时显示数据数值
  const renderDataValues = (analysis: DataAnalysisResult) => {
    ...
    return (
      <div className="mb-6 rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50 to-orange-50 p-6">
        <h3 className="mb-4 flex items-center text-xl font-bold text-amber-900">
          <span className="mr-2">📈</span> 数据数值对比
        </h3>

        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          <div className="space-y-4">
            <div className="rounded-lg bg-white p-4 shadow-sm">
              <div className="mb-2 font-bold text-amber-700">实体A数值</div>
              <div className="space-y-2">
                <div className="flex justify-between">
                  <span className="text-gray-600">当前值:</span>
                  <span className="font-bold text-amber-600">
                    {values?.entityA?.current || 'N/A'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">平均值:</span>
                  <span className="font-bold text-amber-600">
                    {values?.entityA?.mean || 'N/A'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">范围:</span>
                  <span className="font-bold text-amber-600">
                    {values?.entityA?.min || 'N/A'} -{' '}
                    {values?.entityA?.max || 'N/A'}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-4">
            <div className="rounded-lg bg-white p-4 shadow-sm">
              <div className="mb-2 font-bold text-sky-700">实体B数值</div>
              <div className="space-y-2">
                <div className="flex justify-between">
                  <span className="text-gray-600">当前值:</span>
                  <span className="font-bold text-sky-600">
                    {values?.entityB?.current || 'N/A'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">平均值:</span>
                  <span className="font-bold text-sky-600">
                    {values?.entityB?.mean || 'N/A'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">范围:</span>
                  <span className="font-bold text-sky-600">
                    {values?.entityB?.min || 'N/A'} -{' '}
                    {values?.entityB?.max || 'N/A'}
                  </span>
                </div>
              </div>
            </div>
        ...
        {sizeRatio && (
          <div className="mt-4 rounded-lg bg-gradient-to-r from-purple-50 to-pink-50 p-4">
            <div className="flex items-center justify-between">
              <span className="font-semibold text-purple-800">大小比例:</span>
            ...
            <div className="mt-2 text-sm text-gray-600">
              {sizeRatio > 1
                ? `实体A的尺寸是实体B的${sizeRatio.toFixed(1)}倍`
                : `实体B的尺寸是实体A的${(1 / sizeRatio).toFixed(1)}倍`}
            </div>
          </div>
        )}
      </div>
...
"左侧暖色调大尺寸规整形态的[物品A]代表[特征A]，体现[数据特征]；右侧冷色调小尺寸动态形态的[物品B]代表[特征B]，体现[数据特征]；整体极简商务风，高分辨率，真实质感，左右对比清晰，留白充足。"`
      ...
      // 6. 生成数据驱动的对比提示词
      rawContrastPrompt = await generateDataDrivenPrompt(analysis, metaphorType)

      // 备用提示词也更新为数据驱动版本
      fallbackContrastPrompt = getDataDrivenFallbackPrompt(
        analysis,
        metaphorType
      )
              ...
              {isAnalyzingData ? (
                ...
                <div className="space-y-8">
                  ...
                  {dataAnalysis && dataAnalysis.numerical_features && (
                    <div className="mb-6">{renderDataValues(dataAnalysis)}</div>
                  )}
