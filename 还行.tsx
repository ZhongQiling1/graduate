...
function App() {
  ...
  // 在 App 组件中添加或更新这个函数
  const generateContrastPromptWithExplicitObjects = async (
    ...
    const getComparisonObjects = () => {
      if (analysis.comparisonItems && analysis.comparisonItems.length > 0) {
        ...
        } else if (firstItem.includes('对比')) {
          const parts = firstItem.split(/对比|vs/)
          return parts.length >= 2 ? [parts[0], parts[1]] : ['对象A', '对象B']
        }
      }

      // 从特征中提取
      if (analysis.characteristics) {
        const features = Object.keys(analysis.characteristics)
    ...
    const getVisualAttributes = () => {
      ...
      if (analysis.characteristics?.sizeComparison) {
        ...
        if (size.includes('A') && size.includes('大于')) {
          attrs.sizeA = '大尺寸'
          attrs.sizeB = '小尺寸'
        } else if (size.includes('B') && size.includes('大于')) {
          attrs.sizeA = '小尺寸'
          attrs.sizeB = '大尺寸'
        }
      }
    ...
    const visualAttrs = getVisualAttributes()

    // 根据不同隐喻类型生成针对性的提示词
    const getMetaphorSpecificPrompt = () => {
      switch (metaphorType.id) {
        case 1: // 实体隐喻
          return `一张对比图像：左侧是暖色调${visualAttrs.sizeA}${visualAttrs.shapeA}的具体实体"${objectA}"，右侧是冷色调${visualAttrs.sizeB}${visualAttrs.shapeB}的具体实体"${objectB}"。两个实体在同一场景中面对面互动，左侧实体代表数据特征A，右侧实体代表数据特征B。图像中心有明显的视觉分割线，极简商务风格，高分辨率，真实质感，左右对比强烈，图像下方有清晰的标签"${objectA}"和"${objectB}"。`

        case 2: // 结构隐喻
          return `一张建筑结构对比图：左侧是暖色调${visualAttrs.sizeA}${visualAttrs.shapeA}的建筑结构"${objectA}"，右侧是冷色调${visualAttrs.sizeB}${visualAttrs.shapeB}的建筑结构"${objectB}"。两个结构并列展示，中间有明显的视觉分隔，左侧结构稳定有序，右侧结构动态变化。极简建筑风格，高分辨率，光影对比强烈，图像下方标注"${objectA}结构"和"${objectB}结构"。`

        case 3: // 方位隐喻
          return `一张空间布局对比图：画布左侧是暖色调${visualAttrs.sizeA}区域，包含代表"${objectA}"的元素和形态；右侧是冷色调${visualAttrs.sizeB}区域，包含代表"${objectB}"的元素和形态。中间有渐变过渡但分区明显，左侧区域布局规整，右侧区域布局有机。抽象空间艺术风格，高分辨率，层次分明，图像下方标注"${objectA}区域"和"${objectB}区域"。`

        case 4: // 图像隐喻
          return `一张纹理形态对比图：图像左半部分是暖色调${visualAttrs.sizeA}${visualAttrs.textureA}的图案纹理，代表"${objectA}"；右半部分是冷色调${visualAttrs.sizeB}${visualAttrs.textureB}的图案纹理，代表"${objectB}"。左右纹理形成鲜明对比，中间有明显分界。数字艺术风格，高分辨率，真实质感，图像下方标注"${objectA}纹理"和"${objectB}纹理"。`

        default:
          return `一张数据对比图像：左侧暖色调${visualAttrs.sizeA}${visualAttrs.shapeA}的"${objectA}"，右侧冷色调${visualAttrs.sizeB}${visualAttrs.shapeB}的"${objectB}"。两个对象并列对比，中间有明显视觉分隔。极简商务风格，高分辨率，左右对比清晰。`
      }
    }

    const basePrompt = getMetaphorSpecificPrompt()

    // 添加通用要求
    const fullPrompt = `${basePrompt} 图像必须明确展示左右两个对比对象"${objectA}"和"${objectB}"的视觉差异。左侧始终使用暖色调（黄/橙/红），右侧始终使用冷色调（蓝/青/紫）。尺寸差异：${visualAttrs.sizeA} vs ${visualAttrs.sizeB}；形态差异：${visualAttrs.shapeA} vs ${visualAttrs.shapeB}。图像中心有明显的对比分界，但整体协调。简洁商务风格，无多余装饰，高分辨率，真实质感。`
    ...
    try {
      ...
      // 确保包含明确的对比对象
      if (!content.includes(objectA) || !content.includes(objectB)) {
        content = `${content} 图像中明确展示"${objectA}"和"${objectB}"的对比关系。`
      }

      // 确保包含左右分区描述
      if (!content.includes('左侧') || !content.includes('右侧')) {
        content = `左侧"${objectA}" vs 右侧"${objectB}"：${content}`
      }
      ...
      console.error('AI生成对比提示词失败', error)
      // 返回备用提示词
      return `一张明确的数据对比图像：左侧暖色调${visualAttrs.sizeA}${visualAttrs.shapeA}的"${objectA}"对象，右侧冷色调${visualAttrs.sizeB}${visualAttrs.shapeB}的"${objectB}"对象。两个对象在图像中并列展示，中间有明显的视觉分界。左侧代表数据特征A，右侧代表数据特征B。简洁商务风格，高分辨率，真实质感，对比强烈。图像下方标注"${objectA}"和"${objectB}"。`
    }
  }

  // 备用对比提示词 - 优化版（替换原getFallbackContrastPrompt函数）
  // 备用对比提示词 - 同步修复参数
...
"左侧暖色调大尺寸规整形态的[物品A]代表[特征A]，体现[数据特征]；右侧冷色调小尺寸动态形态的[物品B]代表[特征B]，体现[数据特征]；整体极简商务风，高分辨率，真实质感，左右对比清晰，留白充足。"`
      ...
      // 6. 生成对比提示词（全链路存储）
      const rawContrastPrompt = await generateContrastPromptWithExplicitObjects(
        analysis,
        metaphorType
      )
