...
重要要求：
1. 必须使用提供的具体数值特征和行业信息
2. 对比描述要包含具体数值和百分比
3. 行业分析要与提供的行业背景保持一致
4. 确保JSON格式有效
5. 突出显示关键的数值差异和行业特征
6. 隐喻物体选择必须贴合数据类型，禁止所有场景都使用金币：
   - 生物/植物数据：使用花瓣、叶片等植物相关喻体
   - 教育数据：使用书籍、铅笔等教育相关喻体
   - 科技数据：使用芯片、数据流等科技相关喻体
   - 医疗数据：使用药片、心电图等医疗相关喻体
   - 金融数据：可使用金币但也需提供多样化选择`
      } else {
        // 双文件模式
...
请基于以上信息进行分析，返回JSON格式结果，包含详细数值特征和行业分析。
特别要求：隐喻物体选择必须贴合数据类型，避免单一使用金币，根据不同行业选择合适的喻体。`
      }
...
5. 突出关键数值差异
6. 使用中文描述，保持专业性
7. 隐喻物体选择必须泛化，根据数据类型选择合适的喻体，禁止所有场景默认使用金币`
          },
          {
            role: 'user',
            content: prompt
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          // 生成详细的AI解释
          if (mode === 'single') {
            const explainPrompt = `基于以下详细的数据分析结果（特别是具体的数值特征和行业分析），为用户生成一个详细的、易于理解的解释：
...
分析结果：${JSON.stringify(
              {
                ...
                numerical_features: {
                  entityA: analysisResult.numerical_features?.values?.entityA,
                  entityB: analysisResult.numerical_features?.values?.entityB,
                  size_ratio: analysisResult.numerical_features?.size_ratio,
                  percentages: analysisResult.numerical_features?.percentages
                },
...
请重点突出以下内容：
1. 行业背景：${
              analysisResult.industry_analysis?.detected_industry || '通用行业'
            }
2. 具体的数值对比：${
...
3. 百分比差异：${analysisResult.numerical_features?.percent_difference || 0}%
4. 比例关系：${analysisResult.numerical_features?.size_ratio || 1}:1
5. 占比分布：${
              analysisResult.numerical_features?.percentages?.entityA || 0
            }% vs ${
              analysisResult.numerical_features?.percentages?.entityB || 0
            }%
6. 适合的视觉隐喻：${Object.values(
              analysisResult.industry_analysis?.suggested_metaphor_objects || {}
            )
              .map((obj) => obj.primaryObject)
              .join(', ')}

用中文回答，语言要亲切易懂，但必须准确包含具体数值。结构清晰，分段描述。
注意：视觉隐喻的描述要贴合行业特征，避免使用金币作为默认喻体。`
            ...
            try {
              ...
              analysisResult.table_understanding_explained = `基于数据分析，${
                ...
                analysisResult.industry_analysis?.detected_industry || '电商'
              }行业，适合使用${
                analysisResult.industry_analysis?.entity_keywords
                  ?.primary_object || '金币'
              }等实体进行视觉隐喻表达。`
            }
          }
