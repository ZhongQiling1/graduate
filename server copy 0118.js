...

{
  ...
  "analysis_mode": "single"
        ...
        // 分别为两个文件提取特征
        let fileAFeatures = null
        let fileBFeatures = null

        try {
          if (files[0]) {
            fileAFeatures = await extractEnhancedNumericalFeatures(
              files[0].path,
          ...
          if (files[1]) {
            fileBFeatures = await extractEnhancedNumericalFeatures(
              files[1].path,
          ...
          console.warn('单独文件特征提取失败:', featureError)
        }
        ...
        enhancedNumericalFeatures = {
          ...
          values: {
            entityA: {
              label: fileContents[0].filename
                .replace('.csv', '')
                .replace('.json', '')
                .replace('.xlsx', ''),
              mean: fileAFeatures?.values?.entityA?.mean || 1500,
              max: fileAFeatures?.values?.entityA?.max || 2000,
              min: fileAFeatures?.values?.entityA?.min || 1000,
              current: fileAFeatures?.values?.entityA?.current || 1800,
              std: fileAFeatures?.values?.entityA?.std || 250,
              count: fileAFeatures?.values?.entityA?.count || 50,
              unit: fileAFeatures?.values?.entityA?.unit || '数据值'
            },
            entityB: {
              label: fileContents[1].filename
                .replace('.csv', '')
                .replace('.json', '')
                .replace('.xlsx', ''),
              mean: fileBFeatures?.values?.entityA?.mean || 850,
              max: fileBFeatures?.values?.entityA?.max || 1300,
              min: fileBFeatures?.values?.entityA?.min || 600,
              current: fileBFeatures?.values?.entityA?.current || 1000,
              std: fileBFeatures?.values?.entityA?.std || 150,
              count: fileBFeatures?.values?.entityA?.count || 50,
              unit: fileBFeatures?.values?.entityA?.unit || '数据值'
            }
          },
          metadata: {
        ...
        // 分别分析两个文件的行业特征
        let industryInfoA = null
        let industryInfoB = null

        try {
          industryInfoA = await detectIndustryIntelligently(
            fileContents[0].content,
            fileContents[0].filename,
            fileAFeatures || enhancedNumericalFeatures
          )
          industryInfoB = await detectIndustryIntelligently(
            fileContents[1].content,
            fileContents[1].filename,
            fileBFeatures || enhancedNumericalFeatures
          )
          console.log(
            '文件A行业:',
            industryInfoA.industry,
            '文件B行业:',
            industryInfoB.industry
          )
        } catch (industryError) {
          console.warn('行业识别失败:', industryError)
        }
        ...
        // 选择对比更鲜明的行业作为主要行业
        const primaryIndustry = industryInfoA ||
          industryInfoB || {
            industry: '数据对比',
            industry_details: {
              main_category: '数据',
            ...
            entity_keywords: {
              ...
              entityA_synonyms: ['数据集A', '第一组', '文件A'],
              entityB_synonyms: ['数据集B', '第二组', '文件B']
            }
          }
        ...
        metaphorObjects = {
          physical_collections: {
            primaryObject: '数据堆',
            secondaryObjects: primaryIndustry.entity_keywords
              ?.secondary_objects || ['数据点', '记录'],
            colorScheme: {
              warm: ['橙色', '红色', '黄色'],
              cool: ['蓝色', '绿色', '紫色']
            },
            industry: '数据对比',
            entitySynonyms: {
              entityA: ['文件A', '数据集A', '第一组数据'],
              entityB: ['文件B', '数据集B', '第二组数据']
            },
            comparisonHint: `${fileContents[0].filename} vs ${fileContents[1].filename}`
          },
          construction_progress: {
            primaryObject: '数据建筑',
            secondaryObjects: ['数据层', '结构块', '构建单元'],
            colorScheme: {
              warm: ['棕色', '橙色', '红色'],
              cool: ['灰色', '蓝色', '银色']
            },
            industry: '数据对比',
            entitySynonyms: {
              entityA: ['文件A', '数据源A', '左侧数据'],
              entityB: ['文件B', '数据源B', '右侧数据']
            },
            comparisonHint: '数据增长对比'
          },
          density_distribution: {
            primaryObject: '数据点分布',
            secondaryObjects: ['数据集群', '分布点', '密度区'],
            colorScheme: {
              warm: ['红色', '橙色', '粉色'],
              cool: ['蓝色', '紫色', '青色']
            },
            industry: '数据对比',
            entitySynonyms: {
              entityA: ['密集数据A', '文件A数据点'],
              entityB: ['稀疏数据B', '文件B数据点']
            },
            comparisonHint: '数据密度对比'
          }
        }
        ...
        // 增强的双文件对比提示词
        prompt = `你是一个高级数据分析AI。对比分析以下两个数据文件，找出它们之间的核心差异和对比关系。

【文件A详细分析】
文件名: ${fileContents[0].filename}
数据预览: ${fileASummary}
行业分析: ${industryInfoA?.industry || '通用数据'}
数据特征: ${JSON.stringify(
          enhancedNumericalFeatures?.values?.entityA || {},
          null,
          2
        )}
...
数据预览: ${fileBSummary}
行业分析: ${industryInfoB?.industry || '通用数据'}
数据特征: ${JSON.stringify(
          enhancedNumericalFeatures?.values?.entityB || {},
          null,
          2
        )}
...

{
  ...
  "table_understanding": {
    "table_type": "双文件对比",
    "industry_domain": "数据对比分析",
    "business_purpose": "双文件对比评估",
    "content_summary": "${fileContents[0].filename}与${
      fileContents[1].filename
    }的数据对比分析",
    "confidence_score": 0.85,
    "likely_scenarios": ["数据对比", "趋势分析", "差异识别"]
  },
  "comparisonItems": ["${fileContents[0].filename} vs ${
    fileContents[1].filename
  }", "数据规模对比", "趋势差异对比", "波动性对比"],
  "trends": [
    "${fileContents[0].filename}: ${
      enhancedNumericalFeatures.values.entityA.mean
    } ${enhancedNumericalFeatures.values.entityA.unit}，趋势斜率${
      enhancedNumericalFeatures.trend_slopes.entityA
    }%",
    "${fileContents[1].filename}: ${
      enhancedNumericalFeatures.values.entityB.mean
    } ${enhancedNumericalFeatures.values.entityB.unit}，趋势斜率${
      enhancedNumericalFeatures.trend_slopes.entityB
    }%",
    "对比趋势: ${enhancedNumericalFeatures.values.entityA.label}规模是${
      enhancedNumericalFeatures.values.entityB.label
    }的${enhancedNumericalFeatures.size_ratio.toFixed(1)}倍"
  ],
  "characteristics": {
    "sizeComparison": "${fileContents[0].filename}规模是${
      fileContents[1].filename
    }的${enhancedNumericalFeatures.size_ratio.toFixed(1)}倍",
    "volatility": "${fileContents[1].filename}波动性是${
      fileContents[0].filename
    }的${enhancedNumericalFeatures.volatility_ratio.toFixed(1)}倍",
    "percentageDistribution": "${fileContents[0].filename}占比${
      enhancedNumericalFeatures.percentages.entityA
    }%，${fileContents[1].filename}占比${
      enhancedNumericalFeatures.percentages.entityB
    }%",
    "trendDirection": "${fileContents[0].filename}增长${
      enhancedNumericalFeatures.trend_slopes.entityA
    }%，${fileContents[1].filename}增长${
      enhancedNumericalFeatures.trend_slopes.entityB
    }%",
    "valueRange": "${fileContents[0].filename}范围${
  ...
  "summary": "双文件对比分析：${fileContents[0].filename}（${
    enhancedNumericalFeatures.values.entityA.mean
  }${enhancedNumericalFeatures.values.entityA.unit}）与${
    fileContents[1].filename
  }（${enhancedNumericalFeatures.values.entityB.mean}${
    enhancedNumericalFeatures.values.entityB.unit
  }）存在${enhancedNumericalFeatures.size_ratio.toFixed(
    1
  )}倍规模差异，适合进行视觉对比。",
  "numerical_features": ${JSON.stringify(enhancedNumericalFeatures, null, 2)},
  "fileA_analysis": {
    "filename": "${fileContents[0].filename}",
    "industry_domain": "${industryInfoA?.industry || '数据文件'}",
    "content_summary": "${fileASummary.replace(/"/g, "'")}",
    "data_characteristics": "数据规模: ${
      enhancedNumericalFeatures.values.entityA.mean
    }${enhancedNumericalFeatures.values.entityA.unit}，范围: ${
      enhancedNumericalFeatures.values.entityA.min
    }-${enhancedNumericalFeatures.values.entityA.max}",
    "visual_label": "${fileContents[0].filename
      .replace('.csv', '')
      .replace('.json', '')
      .replace('.xlsx', '')}"
  },
  "fileB_analysis": {
    ...
    "industry_domain": "${industryInfoB?.industry || '数据文件'}",
    "content_summary": "${fileBSummary.replace(/"/g, "'")}",
    "data_characteristics": "数据规模: ${
      enhancedNumericalFeatures.values.entityB.mean
    }${enhancedNumericalFeatures.values.entityB.unit}，范围: ${
      enhancedNumericalFeatures.values.entityB.min
    }-${enhancedNumericalFeatures.values.entityB.max}",
    "visual_label": "${fileContents[1].filename
      .replace('.csv', '')
      .replace('.json', '')
      .replace('.xlsx', '')}"
  },
  "file_comparison_summary": {
    ...
    "comparison_type": "数据对比",
    "key_findings": [
      "规模差异: ${enhancedNumericalFeatures.size_ratio.toFixed(1)}倍",
      "占比分布: ${enhancedNumericalFeatures.percentages.entityA}% vs ${
        enhancedNumericalFeatures.percentages.entityB
      }%",
      "趋势差异: ${enhancedNumericalFeatures.trend_slopes.entityA}% vs ${
        enhancedNumericalFeatures.trend_slopes.entityB
      }%"
    ]
  },
  "industry_analysis": {
    "detected_industry": "双文件数据对比",
    "industry_confidence": 0.85,
    "main_category": "数据分析",
    "sub_category": "对比评估",
    "suggested_metaphor_objects": ${JSON.stringify(metaphorObjects, null, 2)},
    "entity_keywords": {
      "primary_object": "对比数据集",
      "secondary_objects": ["数据点", "记录", "指标"],
      "colors": {
        "warm": ["橙色", "红色"],
        "cool": ["蓝色", "绿色"]
      },
      "entityA_synonyms": ["${fileContents[0].filename
        .replace('.csv', '')
        .replace('.json', '')
        .replace('.xlsx', '')}", "文件A", "数据集A"],
      "entityB_synonyms": ["${fileContents[1].filename
        .replace('.csv', '')
        .replace('.json', '')
        .replace('.xlsx', '')}", "文件B", "数据集B"]
    }
  },
  "visualization_suggestions": {
    "main_comparison": "${fileContents[0].filename} vs ${
      fileContents[1].filename
    }",
    "contrast_focus": "规模差异和趋势对比",
    "metaphor_guidance": "使用左右对比的视觉布局，左侧代表${
      fileContents[0].filename
    }，右侧代表${fileContents[1].filename}"
  }
}
